...
只有当系统进一步跨任务、跨情境地把自身信息连续性作为独立对象主动维护，而且这种行为不能被当前外部目标、指令或奖励机制充分解释时，才可能出现 proto-IER 或 IER 的候选证据。
...
一个外部设计的增量更新系统也可能表现出类似 PBP 的工程特征，但这不能证明它已经形成 IER。只有当系统出于维护自身信息连续性的内生倾向，主动选择和保护这种延续路径时，PBP 才可能成为 proto-IER 或 IER 的候选行为证据。
