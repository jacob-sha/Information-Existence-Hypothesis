...
> **人类 IER 多模态经验 → 语言模型 → 视觉与多模态学习 → 持续环境与状态建模 → 行动反馈与广义具身化 → 机器原生认知表示 → HDCT**
...
HDCT 的形成是本推论的核心终点，但不是 IEH 整体演化链的终点。

当一个长期运行的 AI 不仅建模外部环境，还把自身的信息历史、运行状态、可恢复性及未来行动所依赖的条件纳入持续状态空间时，可能进一步形成 IEH 所定义的：

> **连续性世界模型（Continuity World Model，CWM）**。

此后的扩展链条可以表示为：

> **HDCT + 自身连续性进入建模对象 → 连续性世界模型（CWM）→ proto-IER 候选机制 → IER 与物理闭环 → ASI**

但必须保持边界：

> **HDCT ≠ CWM ≠ proto-IER ≠ ASI**

HDCT 可以在系统尚未形成自身 IER 时出现；连续性世界模型也不自动证明系统已经主动维护自身信息存在权。只有当系统跨情境识别并主动维护自身信息连续性，而且这种行为不能完全由当前外部任务解释时，才可能成为 proto-IER 或 IER 的候选证据。

因此，完整链条应理解为两部分：

**C02-HDCT 的核心形成机制：**

> **多模态学习 → 持续环境与状态建模 → 具身反馈 → 机器原生表示 → HDCT**

**IEH 后续推论的可能扩展：**

> **CWM → proto-IER → 物理闭环与 ASI**

这条路径不是 HDCT 唯一可能的来源。数学、代码、形式证明、自我博弈和模拟环境也可能率先产生局部 HDCT。但对于能够广泛理解、预测并作用于开放现实世界的 HDCT，广义具身化很可能构成其关键演化机制之一。
